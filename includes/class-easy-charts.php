...
class Easy_Charts {
	...
	public function get_ec_chart_data( $chart_id = null ) {
		...
		switch ( $ec_chart_type ) {
			case 'ec_bar_chart':
				$ec_chart_type = 'ec_chartjs_chart' === $ec_chart_lib ? 'bar' : 'Bar';
				break;
	...
	public function ec_render_chart( $chart_id = null ) {
		...
		if ( $chart_id ) {
			...
			if ( 'ec_chartjs_chart' === $ec_chart_data['chart_lib'] ) {
				$chart_html  = '<canvas  class="ec-uv-chart-container uv-div-' . $chart_id . '" data-object="ec_object_' . $chart_id . '"></canvas>';
